...
. In the *Customisation > Localisation* tab, choose your capital city. Imager autocompletes the time zone and keyboard layout for that city; you can change these settings. Your choice also sets the Wi-Fi regulatory domain; the regulatory domain always corresponds to the selected capital city. Select *Next*.
+
...
.. If there's no pre-filled information or if you want to change it, enter the SSID (name) of the wireless network. If you don't want your Raspberry Pi to connect to a Wi-Fi network, clear the SSID instead.
.. For secure networks, enter and confirm the password for the network.
