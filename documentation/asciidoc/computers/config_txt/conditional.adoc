...
| `[cm0]`
| Compute Module 0 (also sees `[pi02]` contents)
...
* The "boot variables" are `boot_arg1`, `cust_otpN`, `bootvar0`, `boot_count`, `boot_partition` and `partition`.
...
This enables you to select different configurations based on persistent values stored in customer OTP.

This example `config.txt` snippet changes the `cmdline` file only when bit 0 of the customer OTP row 0 is set:
...
For more information about programming customer OTP rows, see xref:raspberry-pi.adoc#write-and-read-customer-otp-values[Write and read customer OTP values].
